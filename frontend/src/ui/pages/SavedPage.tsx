...
export default function SavedPage() {
  ...
  useEffect(() => {
    api<{ items: Recipe[] }>("GET", "/users/me/saved").then((d) => setItems(d.items)).catch((e) => setErr(e.message));
  }, []);

  return (
    <div className="card">
      <h2>Kaydedilen Tariflerim</h2>
      {err && <div className="error" style={{ marginTop: 12 }}>{err}</div>}
      {items.length === 0 ? (
        <div className="muted">Henüz kaydedilmiş tarif yok.</div>
      ) : (
        <ul>
          {items.map((r) => (
